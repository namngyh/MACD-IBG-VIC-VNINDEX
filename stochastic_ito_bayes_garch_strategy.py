#!/usr/bin/env python3
"""
Single-asset stochastic-calculus strategy with Ito/GBM intuition and Bayes-GARCH risk.
...
that forecast into a long/cash trading rule for the configured asset.
"""
...
def load_vnindex_csv(path: Path) -> pd.DataFrame:
    ...
    first_date = df["date"].dropna().astype(str).iloc[0] if not df.empty else ""
...
def save_forecast_plot(pred: pd.DataFrame, split_date: pd.Timestamp, output_dir: Path) -> None:
    ...
    ax.plot(pred["date"], pred["close"], label="Actual asset close", color="#1f2937", linewidth=1.6)
    ...
    ax.set_title("Asset Forecast: Ito GBM + Bayes-GARCH")
    ax.set_ylabel("Asset close")
    ax.grid(alpha=0.25)
    ax.legend(loc="best")
    fig.tight_layout()
...
def save_backtest_plot(strategy: pd.DataFrame, output_dir: Path, macd_strategy: pd.DataFrame | None = None) -> None:
    ...
    ax.plot(strategy["date"], strategy["benchmark_equity"], label="Buy & hold asset", color="#334155")
...
def save_signal_plot(strategy: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(13, 6))
    ax.plot(strategy["date"], strategy["close"], label="Asset close", color="#1f2937", linewidth=1.4)
...
def save_macd_signal_plot(macd_strategy: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(13, 6))
    ax.plot(macd_strategy["date"], macd_strategy["close"], label="Asset close", color="#1f2937", linewidth=1.4)
    ...
    ax.set_title("MACD(6,26,12) Long/Exit Signals on Asset")
    ax.set_ylabel("Asset close")
    ax.grid(alpha=0.25)
    ax.legend(loc="best")
    fig.tight_layout()
...
def save_macd_indicator_plot(macd_strategy: pd.DataFrame, output_dir: Path) -> None:
    ...
    axes[0].plot(macd_strategy["date"], macd_strategy["close"], color="#1f2937", linewidth=1.3, label="Asset close")
    axes[0].set_title("Asset Close and MACD(6,26,12) Indicator")
...
def save_report(
    ...
    report = f"""# Single-Asset Stochastic Calculus Strategy Report
...
- Signal: long the configured asset when forecast log-return is greater than `{cfg.risk_buffer:.2f}` times forecast daily volatility; otherwise cash.
...
METRIC_LABELS = {
    ...
    "total_points": "Total asset points",
    "annual_points": "Annual asset points",
...
def parse_args() -> StrategyConfig:
    ...
    parser.add_argument("--data", type=Path, default=Path("data.csv"), help="Input OHLCV CSV path")
...
def main() -> None:
    cfg = parse_args()
    summary = run(cfg)
    print("Single-asset stochastic calculus strategy completed.")
